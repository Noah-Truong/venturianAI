...
export const metadata: Metadata = {
  title: "Careers",
  description:
    "Join the team at Venturian AI. We're hiring to build the platform that connects students to weekly income from AI training pipelines.",
  alternates: { canonical: "https://venturian.way-marketing.com/careers" },
};
  ...
  return (
    <>
      ...
      <main id="main-content">
        ...
        <section className="py-32 border-b border-[#D4D4D8]/50">
          <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-[7fr_5fr] gap-16 items-end">
            <FadeIn>
              ...
              <h1
                className="text-5xl sm:text-6xl font-bold text-[#09090B] mb-8"
                style={{ textWrap: "balance" } as React.CSSProperties}
              >
                Build the infrastructure layer for production AI.
              </h1>
              <p className="text-lg text-[#52525B] leading-relaxed max-w-[52ch]">
                Venturian is a small team solving a real problem for students. We move fast, ship constantly,
            ...
            <FadeIn delay={0.1}>
              <div className="rounded-xl border border-[#D4D4D8]/60 bg-[#F4F4F5] p-6 flex flex-col gap-5">
                {[
                  ["51–200", "employees"],
                  ["4", "open roles"],
                  ["2022", "founded"],
                  ...
                  <div key={label} className="flex items-baseline gap-3">
                    <span className="text-2xl font-semibold text-[#09090B] font-mono tabular-nums">
                      {value}
                    </span>
            ...
            >
              {reviews.map((review, i) => (
                <FadeIn key={review.id} delay={Math.min(i * 0.07, 0.35)}>
                  <li>
                    <article className="rounded-xl border border-[#D4D4D8]/60 bg-[#F4F4F5] p-6 h-full flex flex-col gap-5">
                      <Stars rating={review.rating} />
                      <div className="flex flex-col gap-3 flex-1">
                        <h3 className="text-sm font-semibold text-[#09090B] leading-snug">
                          {review.title}
                        </h3>
                        <p className="text-sm text-[#52525B] leading-relaxed">{review.body}</p>
                      </div>
                      <div className="border-t border-[#D4D4D8]/60 pt-4 flex flex-col gap-2">
                        <p className="text-xs text-[#71717A]">
                          <span className="text-[#A1A1AA] font-medium">Pros: </span>
                          {review.pros}
                        </p>
                        <p className="text-xs text-[#71717A]">
                          <span className="text-[#A1A1AA] font-medium">Cons: </span>
                          {review.cons}
                        </p>
                      </div>
                      <div className="flex items-center justify-between text-xs text-[#71717A]">
                        <span className="font-medium text-[#52525B]">{review.role}</span>
        ...
        {/* Open roles */}
        <section id="open-roles" className="py-24 border-b border-[#D4D4D8]/50">
          <div className="max-w-6xl mx-auto px-6">
            <FadeIn>
              <div className="mb-16">
                <p className="text-xs font-medium tracking-widest uppercase text-[#0F766E] mb-4">
            ...
            <ul className="flex flex-col gap-4 list-none" role="list">
              {openRoles.map((role, i) => (
                <FadeIn key={role.title} delay={i * 0.07}>
                  <li>
                    <article className="rounded-xl border border-[#D4D4D8]/60 bg-[#F4F4F5] p-7 flex flex-col gap-4 hover:border-[#D4D4D8] transition-colors">
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div className="flex flex-col gap-1.5">
                          <h3 className="text-[#09090B] font-semibold">{role.title}</h3>
                          <div className="flex flex-wrap gap-3 text-xs text-[#71717A]">
                            <span className="text-[#0F766E]">{role.department}</span>
                        ...
                        <Link
                          ...
                          className="shrink-0 text-sm font-medium px-4 py-2 border border-[#D4D4D8] text-[#09090B] rounded-lg hover:border-[#0F766E] hover:text-[#0F766E] transition-colors min-h-[40px] flex items-center"
                        >
                          Apply
                        </Link>
                      </div>
                      <p className="text-sm text-[#52525B] leading-relaxed">{role.description}</p>
                    </article>
                  </li>
                </FadeIn>
              ))}
        ...
        <section className="py-24">
          <div className="max-w-6xl mx-auto px-6">
            <FadeIn>
              <div className="rounded-xl border border-[#D4D4D8]/60 bg-[#F4F4F5] p-12 flex flex-col gap-6 items-start">
                <p className="text-xs font-medium tracking-widest uppercase text-[#0F766E]">
                  Don't see your role?
                </p>
                <h2
                  className="text-3xl font-semibold text-[#09090B] max-w-[40ch]"
                  style={{ textWrap: "balance" } as React.CSSProperties}
                >
                  We hire for exceptional people before we hire for open headcount.
                </h2>
                <p className="text-[#52525B] leading-relaxed max-w-[52ch]">
                  If you're exceptional at what you do and think Venturian is the right place to
                  do it, send us a note. Describe what you'd work on and why you're the right
                  person to do it.
                </p>
                <Link
                  href="mailto:careers@meridian.ai"
                  className="inline-flex items-center gap-2 text-sm font-medium px-6 py-3 bg-[#0F766E] text-[#09090B] rounded-lg hover:bg-[#0D9488] transition-colors min-h-[44px]"
                >
